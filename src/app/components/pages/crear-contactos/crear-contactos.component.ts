  ...
  createContact(contactForm: NgForm): void {
    ...
    this.contactsService.createContact(this.contact).subscribe({
      next: (data) => {
        ...
        alert(
          `El contacto con nombre: ${this.contact.name}, telefono: ${this.contact.phone}, email: ${this.contact.email} y dirección: ${this.contact.address} ha sido creado exitosamente.`
        );
